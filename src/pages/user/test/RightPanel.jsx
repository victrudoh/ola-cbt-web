...
const RightPanel = () => {
  ...
  console.log(
    "🚀 ~ file: RightPanel.jsx:6 ~ RightPanel ~ oneTest:",
    oneTest?.questions[0]
  );
  ...
  const answerQuestionHandler = async () => {
    ...
    // if (oneTest?.questions?.length < questionIndex + 1) {
    //   console.log(
    //     "🚀 ~ file: RightPanel.jsx:43 ~ answerQuestionHandler ~ oneTest?.questions?.length < questionIndex + 1:",
    //     oneTest?.questions?.length < questionIndex + 1
    //   );
    //   let newIndex = questionIndex + 1;
    //   await setQuestionIndex(newIndex);
    // }
    answerQuestion(oneTest._id, questionId, index);
  };
    ...
    <>
      <div className="bg-white w-[70%] min-h-[50%] rounded-[8px] bg-cover bg-center shadow-xl p-4 flex flex-col items-center justify-start gap-4 z-[1]">
        ...
        <div className="flex gap-6 w-full items-start justify-start my-8 flex-wrap ml-9">
          ...
          <div
            onClick={answerQuestionHandler}
            className="w-[45%] rounded-md py-2 px-4 bg-green-600 text-white hover:bg-teal-100 cursor-pointer flex gap-4 items-center "
          >
            <div className="font-poppins text-xl font-semibold leading-9 tracking-normal text-left text-teal-700">
              B
            </div>
